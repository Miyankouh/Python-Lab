...
class Database:
    ...
    def insert(self, id, name, price, is_food):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "INSERT INTO table_menu VALUES (? , ? , ? , ?)", (id, name, price, is_food))
        self.connection.commit()
        self.connection.close()
    ...
    def get_menu_items(self, is_food):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "SELECT * FROM table_menu WHERE is_food = ?", (is_food,))
    ...
    def get_menu_item_by_name(self, menu_item_name):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "SELECT * FROM table_menu WHERE name = ? ", (menu_item_name,))
        result = self.cursor.fetchall()
        return result
    ...
    def insert_into_receipt(self, receipt_id, menu_id, count, price):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "INSERT INTO table_receipts VALUES(? , ? , ? , ?)", (receipt_id, menu_id, count, price))
        self.connection.commit()
        self.connection.close()
    ...
    def get_receipt_by_receiptid_menuid(self, receipt_id, menu_id):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "SELECT * FROM table_receipts WHERE receipt_id = ? and menu_id = ? ", (receipt_id, menu_id))
        result = self.cursor.fetchall()
        return result
    ...
    def increase_count(self, receipt_id, menu_id):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "UPDATE table_receipts SET count = count + 1 WHERE receipt_id = ? and menu_id = ?", (receipt_id, menu_id))
        self.connection.commit()
        self.connection.close()
    ...
    def get_receipts_by_receiptid(self, receipt_id):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "SELECT * FROM view_menu_receipts WHERE receipt_id = ? ", (receipt_id,))
        result = self.cursor.fetchall()
        return result
    ...
    def delete_receipt(self, receipt_id, menu_id):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "DELETE FROM table_receipts WHERE receipt_id = ? and menu_id = ?", (receipt_id, menu_id))
        self.connection.commit()
        self.connection.close()
    ...
    def decrease_count(self, receipt_id, menu_id):
        ...
        self.cursor = self.connection.cursor()
        self.cursor.execute(
            "UPDATE table_receipts SET count = count - 1 WHERE receipt_id = ? and menu_id = ? and count > 0", (receipt_id, menu_id))

        self.cursor.execute(
            "DELETE FROM table_receipts WHERE receipt_id = ? and menu_id = ? and count = 0", (receipt_id, menu_id))
        self.connection.commit()
        self.connection.close()
...
def load_receipts(receipt_id):
    ...
    for receipt in receipts:
        list_box.insert(0, "%s-%s %s %s" %
                        (receipt[1], receipt[2], receipt[3], receipt[4]))


receipt_frame = LabelFrame(window, text='صورتحساب',
                           font=vazir_font, padx=pad_x, pady=pad_y)
...
entry_order_num = Entry(receipt_frame, font=vazir_font,
...
listbox_buttons_frame.grid(
    row=2, column=0, sticky='nsew', padx=pad_x, pady=pad_y)
...
button_delete = Button(listbox_buttons_frame, text='حذف سطر',
                       font=vazir_font, command=delete_receipt_item)
...
button_new = Button(listbox_buttons_frame, text='فاکتور جدید',
                    font=vazir_font, command=new_receipt)
button_new.grid(row=0, column=1, sticky='nsew')

button_add = Button(listbox_buttons_frame, text='+',
                    font=vazir_font, command=increase_item)
...
button_minus = Button(listbox_buttons_frame, text='-',
                      font=vazir_font, command=decrease_count)
button_minus.grid(row=0, column=3, sticky='nsew')
# endregion

# region منوها
# ********************************************* فریم منو
menu_frame = LabelFrame(window, text='منو نوشیدنی وغذا',
                        font=vazir_font, padx=pad_x, pady=pad_y)
...
def exit_program():
    messag_box = tk.messagebox.askquestion(
        'خروج', 'آیا برای خروج مطمئن هستید؟', icon='warning')
    if messag_box == 'yes':
        window.destroy()
...
button_exit = Button(buttons_frame, text='خروج',
                     font=vazir_font, command=exit_program)
...
button_calculator = Button(
    buttons_frame, text='ماشین حساب', font=vazir_font, command=open_calculator)
button_calculator.grid(row=0, column=1)


def open_website():
    import webbrowser
    webbrowser.open('')  # TODO: your web address


button_open_website = Button(
    buttons_frame, text='وبسایت ما', font=vazir_font, command=open_website)
